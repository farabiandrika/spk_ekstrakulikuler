  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="editEkstrakulikuler" class="form-horizontal form-label-left">
          @csrf
          @method('PUT')
          <div class="form-group">
            <label class="control-label col-md-3 col-sm-3 col-xs-12" for="nama">Nama</label>
            <div class="col-md-6 col-sm-6 col-xs-12">
              <input name="nama" type="text" id="nama" required="required" class="form-control col-md-7 col-xs-12" placeholder="Nama Ekstrakulikuler">
            </div>
          </div>
      </div>
...
<div class="modal fade" id="modal-add" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="addEkstrakulikuler" class="form-horizontal form-label-left">
          @csrf
          <div class="form-group">
            <label class="control-label col-md-3 col-sm-3 col-xs-12" for="nama">Nama</label>
            <div class="col-md-6 col-sm-6 col-xs-12">
...
@section('js')
    ...
    <script>
      $(document).ready(function() {
        ...
        let dataKriteria = $('#data-ekstrakulikuler').DataTable({
          processing: true,
          serverSide: true,
          ajax: "{{ route('ekstrakulikuler.index') }}",
          columns: [
      ...
      $(document).on('click','.edit', function() {
          ...
          $.get("{{ url('/api/ekstrakulikuler') }}" +'/' + id, function (response) {
              $('#modal-edit').modal('toggle')
              $('#editEkstrakulikuler').data("id",id)
              $('#editEkstrakulikuler input[name="nama"]').val(response.data.nama)
          })
      })

      // ADD KRITERIA
     $('#addEkstrakulikuler').submit(function(e) {
        e.preventDefault()
      
        $.ajax({
            method: "POST",
            url: "{{route('ekstrakulikuler.store')}}",
    ...
    $('#editEkstrakulikuler').submit(function(e){
          e.preventDefault()
          let id = $(this).data('id')
  
          $.ajax({
              method: "PUT",
              url: "{{ url('/api/ekstrakulikuler') }}" + "/" + id,
              data: $(this).serialize(),
              dataType: 'JSON',
              error: function(xhr, status, error) {
                  for (variable in xhr.responseJSON.data) {
